...
public class DatabindingObservaleFragment extends Fragment {
    ...
    public class ClickProxy {
        public void nameChange() {
            TestData testData = mViewModel.getTestData().get();
            testData.setName(String.valueOf(System.currentTimeMillis()));
        }

        public void ageChange() {
            TestData testData = mViewModel.getTestData().get();
            testData.setAge(mViewModel.getTestData().get().getAge() + 1);
        }

        public void showChange() {
            TestData testData = mViewModel.getTestData().get();
            testData.setVisible(!mViewModel.getTestData().get().isVisible());
        }

        public void showDoubleBindingResult() {
            //每次都能获取到最新的、由控件变化时自动推到状态变量的状态值
            Toast.makeText(getActivity().getApplicationContext(),
                    mViewModel.getTestData().get().getMsg(), Toast.LENGTH_SHORT).show();
        }
    }
